...
class DeudaOperativaPersonPage extends StatefulWidget {
  const DeudaOperativaPersonPage({super.key});

  @override
  State<DeudaOperativaPersonPage> createState() => _DeudaOperativaPersonPageState();
}
    ...
    return BlocBuilder<MainBloc, MainState>(
      builder: (context, state) {
        List<DeudaOperativaBSingle> lista =
            state.deudaOperativaB?.deudaOperativaPersonSearch ?? [];
        lista =
            lista
                .where(
                  (e) => e.toList().any(
