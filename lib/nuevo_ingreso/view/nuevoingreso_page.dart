...
class _IngresosScreenState extends State<IngresosScreen>
  ...
  void initState() {
    ...
    context.read<MainBloc>().nuevoIngresoCtrl.modifyList(
      index: 3.toString(),
      method: 'resize',
    );
      ...
      appBar: AppBar(
        ...
        actions: [
          ElevatedButton(
            ...
            onPressed: () {
              bool esvalido =
                  context.read<MainBloc>().nuevoIngresoCtrl.validarCampos();
              if (esvalido) {
                Navigator.pop(context);
                context.read<MainBloc>().nuevoIngresoCtrl.enviar();
              }
            },
          ),
        ],
  ...
  Widget gridViewDatosOrigen() {
    return SizedBox(
      ...
      child: BlocBuilder<MainBloc, MainState>(
        builder: (context, state) {
          NuevoIngresoBEncabezado? data = state.nuevoIngresoB?.encabezado;
            ...
            children: [
              ...
              Expanded(
                child: TextField(
                  ...
                  onChanged: (value) {
                    context.read<MainBloc>().nuevoIngresoCtrl.cambiarEncabezado(
                      valor: value,
                      campo: 'codigo_massy',
                    );
                  },
  ...
  Widget addMaterialWidget() {
    return BlocBuilder<MainBloc, MainState>(
      builder: (context, state) {
        ...
        List<NuevoIngresoBSingle>? data = state.nuevoIngresoB?.nuevoIngresoList;
        if (data == null || mm60 == null) return CircularProgressIndicator();
        return ListView.builder(
          shrinkWrap: true,
          itemCount: data.length,
          itemBuilder: (context, index) {
            TextEditingController e4eInit = TextEditingController(
              text: data[index].e4e,
            );
                  ...
                  for (var i = 0; i < data.length; i++) {
                    if (i < rows.length) {
                      ...
                      context
                          .read<MainBloc>()
                          .nuevoIngresoCtrl
                          .cambiarCamposLista(index: i, e4e: values[0]);
                      await Future.delayed(Duration(milliseconds: 100));
                      context
                          .read<MainBloc>()
                          .nuevoIngresoCtrl
                          .cambiarCamposLista(index: i, ctd_e: values[1]);
                      await Future.delayed(Duration(milliseconds: 100));
                    }
                  }
                  // print(rows);
  ...
  Widget singleMaterialRow({
    required int index,
    required NuevoIngresoBSingle data,
    required BuildContext context,
    // required String e4eInit,
  }) {
                  ...
                  return TextFormField(
                    ...
                    onChanged: (value) {
                      context
                          .read<MainBloc>()
                          .nuevoIngresoCtrl
                          .cambiarCamposLista(index: index, e4e: value);
                    },
                  );
                },
              ),
              ...
              child: Builder(
                builder: (context) {
                  ctdController.value = ctdController.value.copyWith(
                    text: data.ctd,
                    selection: TextSelection.collapsed(offset: data.ctd.length),
                  );
                  return TextFormField(
                    ...
                    onChanged: (value) {
                      context
                          .read<MainBloc>()
                          .nuevoIngresoCtrl
                          .cambiarCamposLista(index: index, ctd_e: value);
                    },
                  );
                },
              ),
