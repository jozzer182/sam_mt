...
class DeudaOperativaScreenOrder extends StatefulWidget {
  const DeudaOperativaScreenOrder({super.key});

  @override
  State<DeudaOperativaScreenOrder> createState() => _DeudaOperativaScreenOrderState();
}

class _DeudaOperativaScreenOrderState extends State<DeudaOperativaScreenOrder> {
  String filter = '';
    ...
    return BlocBuilder<MainBloc, MainState>(
      builder: (context, state) {
        List<DeudaOperativaBSingle> lista =
            state.deudaOperativaB?.deudaOperativaBListSearch2 ?? [];
        lista =
            lista
                .where(
                  (e) => e.toList().any(
