...
class DeudaAlmacenScreen extends StatefulWidget {
  const DeudaAlmacenScreen({Key? key}) : super(key: key);

  @override
  State<DeudaAlmacenScreen> createState() => _DeudaAlmacenScreenState();
}

class _DeudaAlmacenScreenState extends State<DeudaAlmacenScreen> {
  String filter = '';
  bool isLoading = true;
    ...
    return Scaffold(
      ...
      floatingActionButton: BlocBuilder<MainBloc, MainState>(
        builder: (context, state) {
          List<DeudaAlmacenBSingle>? lista = state.deudaAlmacenB?.deudaAlmacenB;
          if (lista == null) {
    ...
    return BlocBuilder<MainBloc, MainState>(
      builder: (context, state) {
        List<DeudaAlmacenBSingle> lista =
            state.deudaAlmacenB?.deudaAlmacenBListSearch ?? [];
        lista =
            lista
                .where(
                  (e) => e.toList().any(
