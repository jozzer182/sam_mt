...
class DeudaAlmacenRevScreen extends StatefulWidget {
  const DeudaAlmacenRevScreen({Key? key}) : super(key: key);

  @override
  State<DeudaAlmacenRevScreen> createState() => _DeudaAlmacenRevScreenState();
}

class _DeudaAlmacenRevScreenState extends State<DeudaAlmacenRevScreen> {
  String filter = '';
    ...
    return Scaffold(
      ...
      floatingActionButton: BlocBuilder<MainBloc, MainState>(
        builder: (context, state) {
          List<DeudaAlmacenBSingle>? completo2 =
              state.deudaAlmacenB?.deudaAlmacenB;
          if (completo2 == null) {
  ...
  Widget titleRow() {
    return BlocBuilder<MainBloc, MainState>(
      builder: (context, state) {
        var listaTitulo = state.deudaAlmacenB?.listaTitulo ?? [];
        return Row(
          children: [
  ...
  Widget futureMng() {
    return BlocBuilder<MainBloc, MainState>(
      builder: (context, state) {
        List<DeudaAlmacenBSingle> lista =
            state.deudaAlmacenB?.deudaAlmacenBListSearch2 ?? [];
        lista = lista
            .where(
              (e) => e.toList().any(
